        ...
        try:
            ...
            if available_documents is False:
                if share._check_token(token):
                    ...
                    response = request.render('documents.not_available', options)
                    response.headers['X-Frame-Options'] = 'SAMEORIGIN'
                    logger.info("Share portal rendered for share link id: %s", share_id)    
            ...
            # Xử lý các trường hợp render
            if len(shareable_documents) == 1 and shareable_documents.type == 'empty':
                response = request.render("documents.document_request_page", options)
                ...
                response = request.render('documents.share_workspace_page', options)
            else:
                ...
                response = request.render("documents.share_files_page", options)
