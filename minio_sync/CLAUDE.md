...
This Flutter application fully replaces the legacy Go service (which was previously located at `../service/` but has since been deprecated and removed). Both served the same purpose (local HTTP API for upload operations), but this Flutter version targets cross-platform (desktop + mobile) architectures natively.
